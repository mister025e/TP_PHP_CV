...
// If logged in, fetch CVs for the logged-in user; if not, fetch CVs without a user ID (public).
if ($isLoggedIn) {
    // Fetch CVs for the logged-in user
    $stmt = $pdo->prepare('SELECT * FROM cvs WHERE user_id = :user_id');
    $stmt->execute(['user_id' => $userId]);
} else {
    // Fetch only CVs created by non-logged-in users (public)
    $stmt = $pdo->query('SELECT * FROM cvs WHERE user_id IS NULL');
}
...
<header class="absolute bg-gray-800 text-white text-sm inset-x-0">
  ...
  <div class="relative isolate px-6 pt-14 lg:px-8">
    ...
    <div class="mx-auto max-w-2xl py-32 sm:py-48 lg:py-56">
        <h1 class="text-center text-4xl font-bold tracking-tight text-white">Your CVs</h1>
        <?php if (empty($cvs)): ?>
            <p class="text-center text-white">No CVs available.</p>
        <?php else: ?>
            <ul class="text-white">
                <?php foreach ($cvs as $cv): ?>
                    <li class="mb-4">
                        <a href="view_cv.php?id=<?php echo $cv['id']; ?>" class="text-indigo-300 hover:text-indigo-500">
